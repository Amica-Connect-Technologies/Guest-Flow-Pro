...
const ROLE_META: Record<string, { label: string; bg: string; text: string }> = {
  superuser: { label: "Superuser", bg: "bg-purple-100", text: "text-purple-700" },
  admin:     { label: "Admin",     bg: "bg-blue-100",   text: "text-blue-700"   },
  manager:   { label: "Manager",   bg: "bg-slate-100",  text: "text-slate-600"  },
};
...
export default function AdminUsersPage() {
  ...
  async function handleCreate() {
    if (!form.email.trim() || !form.password.trim()) {
      setForm(f => ({ ...f, error: "Email and password are required." })); return;
    }
    ...
    try {
      ...
      setForm(f => ({ ...f, saving: false, error: e instanceof Error ? e.message : "Failed to create user." }));
    }
  }
    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      <div className="bg-white border-b border-slate-100 px-4 pt-12 pb-4 sticky top-0 z-20">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="font-bold text-slate-900 text-lg">Users</h1>
        ...
        <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
          {["all", ...ROLES].map(r => (
            <button key={r} onClick={() => setFilter(r)}
              ...
              {r === "all" ? `All (${users.length})` : `${ROLE_META[r]?.label ?? r} (${roleCounts[r] ?? 0})`}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 pt-4 space-y-3">
        {filtered.length === 0 ? (
          <div className="bg-white rounded-2xl p-10 text-center border border-slate-100">
            ...
            <p className="font-semibold text-slate-500 text-sm">No users found</p>
          </div>
        ) : filtered.map(u => {
              ...
              <div className="flex items-center gap-3 p-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isMe && <span className="text-[10px] font-bold bg-emerald-100 text-emerald-600 px-1.5 py-0.5 rounded-full">You</span>}
                  </div>
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    <span className="text-[10px] text-slate-300">
                      Joined {new Date(u.date_joined).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {!isMe && (
                <div className="flex border-t border-slate-100">
                  ...
                  <div className="flex-1 flex items-center px-4 py-2.5 gap-2">
                    <span className="text-xs text-slate-400 font-medium flex-shrink-0">Role:</span>
                    <select
                      value={u.role}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm px-0 sm:px-4">
          <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-slate-900 text-lg">Add New User</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block mb-1.5">Password *</label>
                <input
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block mb-1.5">Role</label>
                ...
                <p className="text-xs text-slate-400 mt-1.5">
                  {form.role === "superuser" && "Full system access including all admin controls."}
                  {form.role === "admin" && "Can manage hotels, tours, places, users and registrations."}
                  {form.role === "manager" && "Can manage their assigned hotel's services and bookings."}
                </p>
              </div>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={handleCreate} disabled={form.saving}
                  ...
                  {form.saving ? "Creating…" : "Create User"}
                </button>
              </div>
            </div>
          </div>
