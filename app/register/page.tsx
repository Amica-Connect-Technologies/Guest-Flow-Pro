              ...
              <div key={n} className="flex items-center flex-1 last:flex-none">
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <div className="hidden sm:block">
                    <p className={`text-xs font-bold leading-none ${active ? "text-slate-900" : "text-slate-400"}`}>{s.label}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">{s.subtitle}</p>
                  </div>
                </div>
                {i < visibleSteps.length - 1 && (
                  <div className={`flex-1 mx-3 h-px transition-colors ${done ? "bg-emerald-300" : "bg-slate-200"}`} />
                )}
              </div>
            );
          })}
