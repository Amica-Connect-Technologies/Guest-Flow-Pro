  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-8 w-full max-w-sm text-center">
        ...
        <h1 className="text-xl font-bold text-slate-900 mb-2">{t.registerFlow.paymentCancelled}</h1>
        <p className="text-slate-500 text-sm mb-8">
          {t.registerFlow.cancelledMessage}
        </p>
        <Link
          href="/register"
        ...
        >
          {t.registerFlow.tryAgain}
        </Link>
        <p className="text-slate-400 text-xs mt-4">
          {t.registerFlow.needHelp}{" "}
          <Link href="/contact" className="text-blue-600 hover:underline">{t.registerFlow.contactUs}</Link>
        </p>
      </div>
    </div>
  );
