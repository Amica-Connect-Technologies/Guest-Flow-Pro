...
export default function ConciergeView({ hotelId }: { hotelId: string }) {
  const { t, lang, setHotelLang } = useLanguage();
  useGygImageFallback();
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setHotel(h);
        if (h.language_default) {
          setHotelLang(h.language_default as import("@/lib/i18n").Lang);
        }
  ...
  // ── Plan gating ──────────────────────────────────────────────────────────
  const PLAN_HAS_CONCIERGE = (p: string) => ["concierge", "concierge_checkin", "full"].includes(p);
  if (!PLAN_HAS_CONCIERGE(hotel.plan)) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-6 text-center"
      ...
      <div>
        <p className="font-black text-slate-900 text-2xl mb-2">Digital Concierge Unavailable</p>
        <p className="text-slate-500 text-base max-w-xs mx-auto">
          {hotel.name} has not activated the Digital Concierge service.<br />
          Please contact the hotel reception directly.
        </p>
      </div>
      {hotel.whatsapp_number && (
        <a href={`https://wa.me/${hotel.whatsapp_number.replace(/\D/g, "")}`}
          target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-2.5 px-6 py-4 rounded-2xl text-white font-black text-sm"
          ...
          <div className="flex flex-col md:flex-row md:items-stretch md:gap-10">
            ...
            <div className="flex-1 flex flex-col">
              ...
              {hotel.gallery_images && hotel.gallery_images.length > 0 && (
                <div className="mt-5">
                  <p className="text-[10px] font-black uppercase tracking-widest mb-2.5" style={{ color: "rgba(103,232,249,0.6)" }}>
                    Gallery
                  </p>
                  <div className="flex gap-2.5 overflow-x-auto pb-2" style={{ scrollbarWidth: "none" }}>
                    {hotel.gallery_images.map(img => (
                      <div
                        ...
                        style={{ width: 120, height: 80, border: "1px solid rgba(103,232,249,0.2)" }}
                      >
                        <Image
                          unoptimized
                          src={img.image_url}
              ...
              {/* Book a Room — mobile CTA (hidden on desktop, shown inline) */}
              <div className="md:hidden mt-5">
                <Link href={`/request/${hotel.id}`}
                  className="flex items-center justify-center gap-2.5 w-full py-4 rounded-2xl text-sm font-black text-white transition-all hover:opacity-90"
                  style={{ background: "linear-gradient(135deg, #0891B2, #0E7490)", boxShadow: "0 4px 18px rgba(8,145,178,0.35)", touchAction: "manipulation" }}>
              ...
              <div className="rounded-3xl overflow-hidden flex flex-col w-full"
                ...
                <div className="px-4 pt-4 pb-2">
                  <Link href={`/request/${hotel.id}`}
                    ...
                    style={{ background: "linear-gradient(135deg, #0891B2, #0E7490)", boxShadow: "0 4px 18px rgba(8,145,178,0.35)" }}>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} className="w-4 h-4">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
                    </svg>
                    Book a Room
                  ...
                  <div className="px-4 pb-4">
                    <a href={(() => {
                        const greetings: Record<string, string> = {
                          en: `Hi! I'm a guest at ${hotel.name} and I need some assistance.`,
                          it: `Ciao! Sono un ospite di ${hotel.name} e ho bisogno di assistenza.`,
                          es: `¡Hola! Soy huésped en ${hotel.name} y necesito asistencia.`,
                        };
                        const greeting = greetings[lang] || greetings.en;
                        return `https://wa.me/${hotel.whatsapp_number.replace(/\D/g,"")}?text=${encodeURIComponent(greeting)}`;
                      })()}
                      target="_blank" rel="noopener noreferrer"
                      className="flex items-center justify-center gap-2.5 w-full py-3.5 rounded-2xl text-sm font-black text-white transition-all hover:opacity-90"
                      style={{ background: "linear-gradient(135deg, #10B981, #059669)", boxShadow: "0 4px 18px rgba(16,185,129,0.35)" }}>
